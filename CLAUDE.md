...
## 📊 VERSIÓN ACTIVA: v149
Cambios v149: Corte de caja — validación Dólar igual a Efectivo/Tarjeta. Sección DÓLAR (USD) en ticket ahora tiene Esperado/Contado/DIFERENCIA DÓLAR igual que efectivo y tarjeta (ya no es solo línea en blanco). UI: aparece fila `caja-dolar-row` con input "Dólar contado (MXN)" solo si hubo pagos en Dólar ese día (igual que caja-terminal-row para tarjeta). `calcDiferencia()` calcula diferencia dólar en tiempo real. `imprimirTicketCorte()` acepta `dolarContado` como 10mo parámetro. Fix formato "DIFERENCIA TARJETA / MSI": el texto más largo fuerza wrap a 2 líneas igual que DIFERENCIA EFECTIVO y DIFERENCIA DÓLAR (el texto de 18 chars cabía en 1 línea, el de 24 chars wrappea). Ticket ya solo muestra secciones si hubo ese método de pago.
