...
- **`processLCPhoto` maneja promo**: guarda `[PROMO-OCR] {json}` en `clari_conversations` e invoca Anthropic (claude-opus-4-8 desde v296) con un system prompt específico que incluye las promos vigentes de `getActivePromos()` + los datos extraídos del flyer. Genera respuesta 2-4 líneas comparando vigencia/óptica/condiciones.
