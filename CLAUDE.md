...
**Última versión**: v294 — Hot Sale Car & Era (promo headline de la óptica, 25 may - 2 jun 2026) agregado al prompt de Clari como bloque `hotSaleCarEra` en `getActivePromos()` de ambos webhooks. Combo: 2x1 + lente solar graduado adicional $499, con armazón seleccionado + micas CR-39 + AR + UV INCLUIDOS sin costo extra (esto es la diferencia con el 2x1 estándar). Armazones SELECCIONADOS (no todos los modelos). NO combina con otras promos (maestros, Día del Niño, Aplazo HS2026). Bloque prependido al return string para que sea la primera promo que Clari lee y la anuncie como headline durante los 9 días. Distinción explícita en prompt: Hot Sale Car & Era (este combo de la óptica) vs Hot Sale Aplazo (descuento de la plataforma de pagos, código HS2026, NO de la óptica) — son DOS Hot Sales paralelos. Anterior v293: Hot Sale Aplazo + reglas anti-coqueteo en DEFAULT_PERSONALITY.
