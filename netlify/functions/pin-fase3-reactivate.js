// Pinocelli Fase 3 Reactivation — Clientes dormidos 2023 + gasto medio/bajo 2024
// Reutiliza templates VIP (misma estrategia: revisar graduación, sin citas)
// Manual: GET /.netlify/functions/pin-fase3-reactivate?key=SECRET
// Dry run: GET /.netlify/functions/pin-fase3-reactivate?key=SECRET&dry=1
    ...
    for (const c of limited) {
      ...
      if (ok) {
        await saveToHistory(c.phone, 'assistant',
          `[${TAG}] [Template-${group}] Reactivación Pinocelli Fase 3 enviado a ${c.name}`
        );
        enviados++;
