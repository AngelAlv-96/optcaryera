...
function getActivePromos() {
  ...
  // 🎟️ CUPÓN "lente solar graduado gratis" (campaña de reactivación). Vence el 30 de junio 2026 (después se envía otro cupón). Convierte el 2x1 en 3x1.
  ...
  var cuponSolar3x1 = cuponSolar3x1Active ? (
    ...
    '• Durante esta campaña, el lente solar graduado que normalmente cuesta $499 adicional va GRATIS con el cupón.\n' +
    ...
    '• El cliente debe PRESENTAR EL CUPÓN en sucursal. Si dice que recibió el cupón / "el del lente solar gratis" / manda la imagen → confírmaselo e invítalo a pasar HOY 30 de junio (último día). Aplica en las 4 sucursales.\n' +
    '• NO combinable con otros descuentos. El precio del 2x1 depende del armazón/graduación/material que elija (no des número).\n' +
  ...
  // pero dejarlo en BLANCO fue un error: el 6-ago una clienta preguntó por él y Clari improvisó
  // — le dijo que valía en las 4 sucursales (era SOLO Vittoria) y que seguía vigente (llevaba 6
  // días vencido). Cuando falta la regla, el modelo rellena. Ahora se le dice qué NO puede afirmar.
  var cuponVittoria30 =
...
// ── 🎟️ CUPÓN 10% A PETICIÓN (v592) — espejo del de WhatsApp ──
// SOLO si el cliente lo pide explícitamente. El código lo acuña el SISTEMA, no el modelo.
// En Messenger/IG el "teléfono" es el PSID: sirve igual como llave de "uno por persona".
...
// Rescate: "ya fui pero se me hizo caro". Necesita las DOS señales (que ya fue Y que le
// pareció caro): solo con "está caro" no se manda, o lo regalaríamos a todos.
...
async function darCuponClari(senderId, userName, motivo) {
  var prev = await supaFetch('cupones?campana=eq.' + CUPON_CLARI_CAMPANA + '&telefono=eq.' + senderId + '&select=codigo,usado,vigencia&limit=1');
  ...
  for (var intento = 0; intento < 3 && !codigo; intento++) {
    ...
    var ins = await supaFetch('cupones', { method: 'POST', body: JSON.stringify({
      ...
      telefono: senderId, nombre: userName || null, vigencia: vig,
      aplica_a: 'lentes_completos', enviado_at: new Date().toISOString()
    }) });
    if (ins && ins[0] && ins[0].codigo) codigo = ins[0].codigo;
  }
  ...
  try {
    ...
    for (var e = 0; e < body.entry.length; e++) {
      ...
      for (var m = 0; m < messaging.length; m++) {
        ...
        // Determinista: el modelo no acuña códigos ni decide a quién dárselo.
        var _cupMotivoM = pideCuponExplicito(messageText) ? 'peticion' : (objecionPrecioTrasVisita(messageText) ? 'precio' : null);
        if (_cupMotivoM) {
          await saveMessage(senderId, 'user', '[Vía ' + channel + '] ' + messageText, senderName || ('clari-' + channel), channel);
          var cupReplyM;
          try { cupReplyM = await darCuponClari(senderId, senderName, _cupMotivoM); }
          catch (eCupM) {
            console.error('[CuponClari Meta]', eCupM.message);
            cupReplyM = 'Claro que sí 🎟️ Tuve un detalle técnico generando tu código — escríbeme de nuevo en un momento y te lo mando.';
          }
          await sendMetaReply(senderId, cupReplyM, channel);
          await saveMessage(senderId, 'assistant', '[Cupon-Clari-10] ' + cupReplyM, null, channel);
          console.log('[CuponClari] ' + channel + ' ' + senderId);
          continue;
        }
