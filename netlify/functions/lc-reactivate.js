...
exports.handler = async function(event) {
  ...
  try {
    ...
    for (const c of limited) {
      ...
      // Re-check dedup PER-FONO inmediatamente antes de enviar (protege contra race entre invocaciones)
      try {
        const recheck = await supaREST('GET',
          `clari_conversations?phone=eq.${c.phone}&content=ilike.*LC-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=id&limit=1`
        );
        if (recheck && recheck.length > 0) {
          console.log(`[LC-REACTIVATE] ⚠ Skip ${c.phone.slice(-4)} — ya enviado (race check)`);
          continue;
        }
      } catch (e) {
        console.warn('[LC-REACTIVATE] Re-check falló, salto por seguridad:', e.message);
        continue;
      }
