// PIN LC Reactivation — Envía WA a usuarios de LC dormidos con A/B/C test
// Lee lista de contactos de app_config id='pin_lc_contacts'
...
async function sendTemplate(to, templateSid, variables) {
  ...
  try {
    ...
    if (data.error_code) {
      console.error(`[PIN-LC] WA error ${data.error_code}: ${data.message}`);
      return false;
    }
    return true;
  } catch (e) {
    console.error('[PIN-LC] WA exception:', e.message);
    return false;
  }
}
...
async function saveToHistory(phone, role, content) {
  try {
    await supaREST('POST', 'clari_conversations', {
      ...
      role,
      content,
      user_name: 'pin-lc-reactivacion'
    });
  } catch (e) { console.error('[PIN-LC] Save history error:', e.message); }
}
...
exports.handler = async function(event) {
  ...
  console.log(`[PIN-LC] Inicio${dryRun ? ' (DRY RUN)' : ''}`);
  ...
  if (horaLocal < 10 || horaLocal >= 20) {
    console.log(`[PIN-LC] ⏰ Fuera de horario (${horaLocal}:${String(nowCH.getMinutes()).padStart(2, '0')} Chihuahua)`);
  ...
  try {
    ...
    let contactList = [];
    try {
      const cfgResp = await supaREST('GET', "app_config?id=eq.pin_lc_contacts&select=value");
      if (cfgResp && cfgResp[0]) {
        contactList = JSON.parse(cfgResp[0].value);
      }
    } catch (e) {
      console.error('[PIN-LC] Error cargando contactos:', e.message);
    }
    ...
    console.log(`[PIN-LC] ${contactList.length} contactos en lista`);
    ...
    for (let i = 0; i < phones.length; i += 20) {
      ...
      try {
        const msgs = await supaREST('GET',
          `clari_conversations?phone=in.(${phoneFilter})&content=ilike.*PIN-LC-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=phone&limit=100`
        );
        if (msgs) msgs.forEach(m => alreadySent.add(m.phone));
      } catch (e) { /* continue */ }
    }

    console.log(`[PIN-LC] ${alreadySent.size} ya contactados en últimos ${DEDUP_DAYS} días`);

    // Exclude recent buyers — DISABLED (verified manually before each campaign launch)
    const recentBuyers = new Set();

    console.log(`[PIN-LC] ${recentBuyers.size} compraron recientemente (excluidos)`);
    ...
    console.log(`[PIN-LC] ${toSend.length} elegibles, enviando ${limited.length} (máx ${MAX_PER_RUN})`);
    ...
    for (const c of limited) {
      ...
      if (!templateSid) {
        console.warn(`[PIN-LC] No template SID for group ${group}`);
        continue;
      }
      ...
      try {
        const recheck = await supaREST('GET',
          `clari_conversations?phone=eq.${c.phone}&content=ilike.*PIN-LC-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=id&limit=1`
        );
        if (recheck && recheck.length > 0) {
          console.log(`[PIN-LC] ⚠ Skip ${c.phone.slice(-4)} — ya enviado (race check)`);
          continue;
        }
      } catch (e) {
        console.warn('[PIN-LC] Re-check falló, salto por seguridad:', e.message);
        continue;
      }
      ...
      if (ok) {
        await saveToHistory(c.phone, 'assistant',
          `[PIN-LC-Reactivacion] [Template-${group}] Mensaje de reactivación LC Pinocelli enviado a ${c.name}`
        );
        enviados++;
        resultados[group].enviados++;
        console.log(`[PIN-LC] ✓ ${nombre} (${c.phone.slice(-4)}) [${group}]`);
      }
    ...
    console.log(`[PIN-LC] Completado: ${enviados}/${limited.length}`);
    ...
    if (enviados > 0) {
      ...
      for (const ap of adminPhones) {
        ...
        } catch (e) { console.warn('[PIN-LC] Admin notify error:', e.message); }
      }
    }
    ...
    console.error('[PIN-LC] Fatal:', err.message);
