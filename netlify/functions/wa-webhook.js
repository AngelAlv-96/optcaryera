...
function getActivePromos() {
  ...
  var hotSaleCarEra = comboActive ? (comboHeader +
    '• Es EN LENTES COMPLETOS (armazón + micas graduadas) 👓. ✅ El ANTIRREFLEJANTE y la GRADUACIÓN de visión sencilla YA van INCLUIDOS (no se cobran como un extra aparte). ⛔ NUNCA digas que "el antirreflejante se cobra aparte" ni que "es opcional con costo" — es INCORRECTO y contradice el anuncio. ⚠️ PERO "incluido" NO significa que el cliente solo pague el armazón: el precio es de los LENTES COMPLETOS (armazón + micas con graduación y antirreflejante) y se PERSONALIZA según el armazón, el material y el tipo de graduación — por eso no hay precio fijo.\n' +
    ...
    '• El precio sube según la graduación bifocal/progresivo, el material (policarbonato/alto índice) y otros tratamientos (blue light, transitions/fotocromático, polarizado). El ANTIRREFLEJANTE NO sube el precio: ya va incluido. En sucursal le arman la cotización exacta. NO des un precio fijo de la promo.\n' +
  ...
  if (year === 2026 && (month === 4 || month === 5 || month === 6 || month === 7)) {
    return cuponMexico + cuponSolar3x1 + hotSaleCarEra + campanaFotoColor + 'PROMOCIÓN VIGENTE:\n' +
      ...
      '• El ANTIRREFLEJANTE y la GRADUACIÓN de visión sencilla SÍ van incluidos (sin cargo aparte) — así lo dice el anuncio. Lo que NO es fijo es el precio TOTAL: el armazón, el material (CR-39/policarbonato/alto índice) y el tipo de graduación (sencilla/bifocal/progresivo) los elige el cliente y eso define el precio. NO prometas un material premium (poli/alto índice) "incluido" — esos suben el precio.\n' +
      ...
      '• El antirreflejante va INCLUIDO (no sube el precio). Otros tratamientos opcionales (blue light, transitions/fotocromático, polarizado) sí suben precio aparte.\n' +
      ...
      '• Si preguntan "¿qué está incluido?": la promoción incluye ARMAZÓN + MICAS con GRADUACIÓN + ANTIRREFLEJANTE (esos van incluidos, sin cargo aparte). ⚠️ Si el cliente cree que solo paga el armazón y todo lo demás va gratis encima, aclara: "El antirreflejante y tu graduación van incluidos (no se cobran aparte), pero el precio es de tus lentes COMPLETOS (armazón + micas) y se cotiza según el armazón, el material y el tipo de graduación que elijas — no es solo el precio del armazón." El material premium (policarbonato/alto índice), la graduación bifocal/progresivo y otros tratamientos (blue light, fotocromático, polarizado) suben el precio. El 3er lente solar es aparte (gratis en compras >$3,000, o $499 si es menor).\n' +
      '• ⚠️ La promoción SIEMPRE incluye el antirreflejante — NO aplica a lentes SIN antirreflejante. No se puede quitar el antirreflejante para pagar menos: el AR es parte de la promo.\n' +
