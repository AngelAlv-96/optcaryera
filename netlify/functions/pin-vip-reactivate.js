    ...
    for (const c of limited) {
      ...
      try {
        const recheck = await supaREST('GET',
          `clari_conversations?phone=eq.${c.phone}&content=ilike.*PIN-VIP-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=id&limit=1`
        );
        if (recheck && recheck.length > 0) {
          console.log(`[PIN-VIP] ⚠ Skip ${c.phone.slice(-4)} — ya enviado (race check)`);
          continue;
        }
      } catch (e) {
        console.warn('[PIN-VIP] Re-check falló, salto por seguridad:', e.message);
        continue;
      }
