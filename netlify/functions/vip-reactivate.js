// VIP Reactivation — Envía WA a clientes VIP dormidos (gasto alto, sin compra en 1+ año)
...
async function sendTemplate(to, templateSid, variables) {
  ...
  try {
    ...
    if (data.error_code) {
      console.error(`[VIP-REACTIVATE] WA error ${data.error_code}: ${data.message}`);
      return false;
    }
    return true;
  } catch (e) {
    console.error('[VIP-REACTIVATE] WA exception:', e.message);
    return false;
  }
}
...
async function saveToHistory(phone, role, content) {
  try {
    await supaREST('POST', 'clari_conversations', {
      ...
      role,
      content,
      user_name: 'vip-reactivacion'
    });
  } catch (e) { console.error('[VIP-REACTIVATE] Save history error:', e.message); }
}
...
async function sendAdminWA(msg) {
  ...
  for (const ap of phones) {
    ...
    } catch (e) { console.warn('[VIP-REACTIVATE] Admin notify error:', e.message); }
  }
}
...
exports.handler = async function(event) {
  ...
  console.log(`[VIP-REACTIVATE] Inicio${dryRun ? ' (DRY RUN)' : ''}`);
  ...
  if (horaLocal < 10 || horaLocal >= 20) {
    console.log(`[VIP-REACTIVATE] Fuera de horario (${horaLocal}h Chihuahua)`);
  ...
  try {
    ...
    let contactList = [];
    try {
      const cfgResp = await supaREST('GET', "app_config?id=eq.vip_reactivacion_contacts&select=value");
      if (cfgResp && cfgResp[0]) {
        contactList = JSON.parse(cfgResp[0].value);
      }
    } catch (e) {
      console.error('[VIP-REACTIVATE] Error cargando contactos:', e.message);
    }
    ...
    console.log(`[VIP-REACTIVATE] ${contactList.length} contactos`);
    ...
    for (let i = 0; i < phones.length; i += 20) {
      ...
      try {
        const msgs = await supaREST('GET',
          `clari_conversations?phone=in.(${phoneFilter})&content=ilike.*VIP-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=phone&limit=200`
        );
    ...
    console.log(`[VIP-REACTIVATE] ${toSend.length} elegibles, enviando ${limited.length}`);
    ...
    for (const c of limited) {
      ...
      if (!templateSid) {
        console.warn(`[VIP-REACTIVATE] No template for group ${group}`);
        continue;
      }
      ...
      try {
        const recheck = await supaREST('GET',
          `clari_conversations?phone=eq.${c.phone}&content=ilike.*VIP-Reactivacion*&created_at=gte.${dedupFrom.toISOString()}&select=id&limit=1`
        );
        if (recheck && recheck.length > 0) {
          console.log(`[VIP-REACTIVATE] ⚠ Skip ${c.phone.slice(-4)} — ya enviado (race check)`);
          continue;
        }
      } catch (e) {
        console.warn('[VIP-REACTIVATE] Re-check falló, salto por seguridad:', e.message);
        continue;
      }
      ...
      if (ok) {
        await saveToHistory(c.phone, 'assistant',
          `[VIP-Reactivacion] [Template-${group}] Mensaje de reactivación VIP enviado a ${c.name}`
        );
        enviados++;
        resultados[group].enviados++;
        console.log(`[VIP-REACTIVATE] ✓ ${nombre} (..${c.phone.slice(-4)}) [${group}]`);
      }

      await new Promise(resolve => setTimeout(resolve, 1500));
    }

    console.log(`[VIP-REACTIVATE] Completado: ${enviados}/${limited.length}`);
    ...
    console.error('[VIP-REACTIVATE] Fatal:', err.message);
